...
import { DESCRIPTION_MAX_LENGTH, RADIUS } from '../constants/theme';
...
export function TaskCard({ task, onDelete, onEdit }) {
  ...
  const safeDescription = (task.description || '').substring(0, DESCRIPTION_MAX_LENGTH);
  ...
  return (
    <View style={[styles.card, { backgroundColor: colors.container }]}>
      ...
      <View style={styles.topActions}>
        ...
        >
          <Pencil size={18} color={colors.iconText} strokeWidth={2.5} />
        </Pressable>
        ...
        <Pressable 
          ...
          style={({ pressed }) => [styles.iconBtn, pressed && styles.pressed]}
        >
          <X size={20} color={colors.iconText} strokeWidth={2.5} />
        </Pressable>
      </View>
      ...
      <View style={styles.contentCenter}>
        ...
        <Text style={[styles.cardSubject, { color: colors.titleText }]}>
          {task.subject.toUpperCase()}
        </Text>
        <Text style={[styles.cardDate, { color: colors.titleText }]}>
          FECHA DE ENTREGA: {task.dueDate || '—'}
        </Text>
      </View>

      {expanded && (
        <Text style={[styles.description, { color: colors.titleText }]}>
          {safeDescription}
        </Text>
      )}
      ...
      <Pressable onPress={toggle} style={styles.chevronBtn}>
        {expanded ? (
          <ChevronUp size={22} color={colors.iconText} strokeWidth={2.5} />
        ) : (
          <ChevronDown size={22} color={colors.iconText} strokeWidth={2.5} />
        )}
      </Pressable>
    </View>
  );
