...
export default function RegisterScreen({ onRegisterSuccess, onGoToLogin }) {
  ...
  const handleRegister = useCallback(async () => {
    ...
    setLoading(true);
    const result = await registerUser({ email: e, password: p });
    setLoading(false);

    if (!result.success) {
      Alert.alert('Error al registrarse', result.message);
      return;
    }

    Alert.alert('Registro completado', result.message, [
      {
  ...
  }, [username, email, password, loading, onRegisterSuccess]);
